...
/*   Updated: 2024/07/25 19:27:58 by hubourge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	for (unsigned int i = 0; i < channels.size() - 1; i++) // channels available
	{
		for (unsigned int j = 0; j < chnlName.size() - 1; j++) // channel to kick
		{
			if (channels[i]->getName() == chnlName[j]) // channel found
			{
				...
				{
					if (channels[i]->getOperator()->getNick() != client.getNick()) // is not operator
					{
						writeError(client, 0, 482, "KICK :More privileges needed");
						continue ;
					}
